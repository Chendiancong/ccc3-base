...
export class AudioManager extends Component {
    ...
    onLoad() {
        ...
        this._uiEffectPlayer = new EffectPlayer(this);//SingleEffectPlayer
...
class MusicPlayer extends CorePlayer {
    ...
    private _getCache(manager: AudioManager, option: PlayAudioOption) {
        let cache = this._caches.get(option.clip.name);
        if (cache == void 0)
            cache = this.createCache(manager, option, MusicCache);
        cache.setup(manager, option);
        this._caches.add(cache.clipName, cache);
        return cache;
    }
}
...
class SingleEffectPlayer extends CorePlayer {
    ...
    play(manager: AudioManager, option: PlayAudioOption) {
        ...
        if (cache) {
            ...
            this._caches.add(option.clip.name, cache);
...
class EffectPlayer extends CorePlayer {
    ...
    play(manager: AudioManager, option: PlayAudioOption) {
        ...
        if (!isValid(option.clip))
            return;
        let caches = this._allCaches[option.clip.name];
        if (caches == void 0)
            caches = this._allCaches[option.clip.name] = {
                freeIndice: [],
    ...
    private _onEnd(cache: AudioCache) {
        delete cache.onEnd;
        const caches = this._allCaches[cache.clipName];
...
class AudioCache {
    ...
    protected calcScaledVolume(manager: AudioManager, oldScale: number, _: number) {
