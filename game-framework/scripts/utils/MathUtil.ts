import { EPSILON, Quat, Vec2, Vec3 } from "cc";
 ...
 * maxDelta大于0时表示接近，小于0时表示远离
 */
function vec3MoveTowards(cur: Vec3, target: Vec3, maxDelta: number, out?: Vec3) {
 ...
 * maxDelta大于0时表示接近，小于0时表示远离
 */
function vec3RotateTowards(cur: Vec3, target: Vec3, maxDelta: number, out?: Vec3) {
    const _cur = tempV3.set(cur);
    const _target = tempV3_2.set(target);
    const _dir = tempV3_3;
    out = out ?? new Vec3();
